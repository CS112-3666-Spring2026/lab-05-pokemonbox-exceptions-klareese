...
public class Pokemon {
    ...
    public Pokemon(String name, String type1, String type2) {
        if (!this.setAll(name, type1, type2)) {
            throw new IllegalArgumentException(
                "Invalid arguments: name/type1/type2 must be valid Pokémon values."
            );
        }
    }
    ...
    public Pokemon(Pokemon p) {
        if (p == null) {
            throw new IllegalArgumentException(
                "Copy constructor error: Pokemon cannot be null."
            );
        }

        if (!this.setAll(p.name, p.type1, p.type2)) {
            throw new IllegalArgumentException(
                "Copy constructor error: invalid Pokémon data."
            );
        }
    }
    ...
    private String toTitleCase(String s) {
        ...
        for (String word : words) {
            builder.append(" ").append(this.toTitleCaseWord(word));
        }
